            ...
            <div class="bg-white p-6 shadow sm:rounded-lg">
                <form action="{{ route('tasks.store') }}" method="POST">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="text" name="title" id="title" class="mt-1 block w-full rounded border-gray-300 shadow-sm focus:ring-blue-500 focus:border-blue-500" value="{{ old('title') }}">
                        @error('title') <p class="text-sm text-red-500">{{ $message }}</p> @enderror
                    </div>

                    <div class="mb-4">
                        <label for="description" class="block text-sm font-medium text-gray-700">Descrição</label>
                        <textarea name="description" id="description" rows="3" class="mt-1 block w-full rounded border-gray-300 shadow-sm focus:ring-blue-500 focus:border-blue-500">{{ old('description') }}</textarea>
                    ...
                    <div class="mb-4">
                        ...
                        <select name="statuses_id" id="statuses_id" class="mt-1 block w-full rounded border-gray-300 shadow-sm focus:ring-blue-500 focus:border-blue-500">
                            ...
                            @foreach ($statuses as $status)
                                <option value="{{ $status->id }}" {{ old('statuses_id') == $status->id ? 'selected' : '' }}>
                                    {{ $status->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="flex justify-end">
                        <a href="{{ route('tasks') }}" class="px-4 py-2 mr-4 text-gray-600 align-items-center hover:underline">Cancelar</a>
